...
export default function AddStudent() {
  ...
  const { id } = useParams()
  // hook này sẽ tham chiếu đến queryClient trong file index
  const queryClient = useQueryClient()

  const matchAdd = useMatch('/students/add')
  const isAddMode = Boolean(matchAdd)
  ...
  const studentQuery = useQuery({
    ...
    // Do trước đó đã prefetching thì mà staleTime chưa hết thì nó sẽ ko thể nhảy vào onSuccess --> ko cung dữ liệu trên Form
    // onSuccess: (data) => {
    //   setFormState(data.data)
    // }
  })

  // Muốn cập nhật cho dữ liệu trên form
  useEffect(() => {
  ...
  const updateStudentMutation = useMutation({
    ...
    onSuccess: (data) => {
      // Cập nhật lại dữ liệu mới nhất cho tool khi mà update
  ...
  const handleChange = (name: keyof FormStateType) => (event: React.ChangeEvent<HTMLInputElement>) => {
    const { data, error, reset } = addStudentMutation
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isAddMode) {
      ...
      // Cach 2
      // Thực hiện gửi dữ liệu để thêm Student, và khi mà dữ liệu được phản hồi trở lại thì reset lại các giá trị form về rỗng
